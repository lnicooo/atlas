...
public class Graph3D{
    ...
    public void generate(String type){
		...
		if(type.equalsIgnoreCase("Number of flits transmitted")){
			...
			writeTxt(name);
			if(noc.getType().equals("HermesTB"))
				writeDatNFTTorus(name);
			else
				writeDatNFT(name);
		}
	...
	private void writeTxt(String name){
	    ...
	    if(noc.getType().equals("HermesTB")){
            	aux = aux + "set xlabel \"X traffic (x = par, y = ímpar)\"\n";
            	aux = aux + "set ylabel \"Y traffic (x = ímpar, y = par)\"\n";
	    }else{
		aux = aux + "set xlabel \"X traffic (x = ímpar, y = par)\"\n";
            	aux = aux + "set ylabel \"Y traffic (x = par, y = ímpar)\"\n";
            } 
	 ...
	 * @param name The name file.
	 */
	private void writeDatNFTTorus(String name){
		...
		try{
          		  FileOutputStream fos = new FileOutputStream(dirSaida+File.separator + "reports" + File.separator + "dat" + File.separator+name+".dat");
           		  OutputStreamWriter osw = new OutputStreamWriter(fos);
            		  Writer out = new BufferedWriter(osw);
           		  //roteadores
            		  for(int y=1; y<=(numRotY*2); y=y+2){
				for(int x=1; x<=(numRotX*2); x=x+2){
                    			aux = x + " " + y + " 0\n";
                    			out.write(aux);
				}
            		  }
            		  //espacos vazios
            		 for(int y=0; y<=(numRotY*2); y=y+2){
				for(int x=0; x<=(numRotX*2); x=x+2){
                    			aux = x + " " + y + " 0\n";
                    			out.write(aux);
				}
           		  }
	    		 int x=0, y=1, first=0,	last=numRotX-1;
	    		 //anéis horizontais
	  		 for(int i=0; i<numRot; i=i+numRotX){
 				if((new File(dirEntrada.concat(File.separator + "r"+i+"p1.txt"))).exists()){
					aux = x + " " + y + " " + (canal[first][1].getNumberOfFlits()+canal[last][0].getNumberOfFlits()) + "\n";
					out.write(aux);
                   		}
				if((new File(dirEntrada.concat(File.separator + "r"+last+"p0.txt"))).exists()){
					x=numRotX*2;
					aux = x + " " + y + " " + (canal[last][0].getNumberOfFlits()+canal[first][1].getNumberOfFlits()) + "\n";
					out.write(aux);
                   		}
			        x=0;
				y=y+2;
			...
			last=numRot-numRotX;
	  		//anéis verticais
	   		for(int i=0; i<numRotX; i++){
 				if((new File(dirEntrada.concat(File.separator + "r"+i+"p3.txt"))).exists()){						
					aux = x + " " + y + " " + (canal[first][3].getNumberOfFlits()+canal[last][2].getNumberOfFlits()) + "\n";
					out.write(aux);
                   		}				
				if((new File(dirEntrada.concat(File.separator + "r"+last+"p2.txt"))).exists()){
					y=numRotY*2;				
					aux = x + " " + y + " " + (canal[last][2].getNumberOfFlits()+canal[first][3].getNumberOfFlits()) + "\n";
					out.write(aux);
                   		}
			        x=x+2;
				y=0;
