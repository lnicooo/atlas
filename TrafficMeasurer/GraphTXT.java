...
/**
	 * Write the TXT file that contains the data showed in the "Number of flits transmitted" graph for Torus TU and Torus TB topologies.  
	 * @return The name file.
	 */
   private String writeTxtTorusNBT(){
		String aux = new String();
		int cont = (numRotX*numRotY)-(numRotX);
		int cont2 = cont;
		String espacos ="                  ";
		...
		try{
			...
			for(int contador = 0; contador<numRotY; contador++){
				aux = aux + "         ";								
           			for(int i=0; i<numRotX; i++){
               				cont = cont2+i;
					aux = aux + geraCarga(cont, 2) + espacos;
				}	
				out.write(aux);			
				aux = "\n";
				for(int i=0; i<numRotX; i++){
				...
				aux = aux + "         ";
           			for(int i=0; i<numRotX; i++){
               				cont = cont2+i;				
					aux = aux + geraCarga(cont, 3) + espacos;
				}
				out.write(aux);	
				aux = "\n\n";
				cont++;
				cont2 = cont - (2*numRotX);				
			}	         			
            		out.close();
            		osw.close();
            		fos.close();
