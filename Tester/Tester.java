 ...
 **********************************************************************************************************/
	...
	private static Scenery createScenery(Project project, Map<String, String> map) throws Exception{
	    ...
	    if(noc.getType().equalsIgnoreCase(NoC.MERCURY) || noc.getType().equalsIgnoreCase(NoC.HERMESSR))
			map.put("InternalSimulation", Boolean.toString(false));
		    	...
		    	if(nLines == nScenery){
					...
					if(nTokens!=0){
						for(int i=0; i<nTokens; i++){
							...
							switch(i){
							case 0 : map.put("Name", token); break;
							case 1 : map.put("InternalSimulation", token); break;
							case 2 : map.put("SimulationTime", token); break;
							case 3 : map.put("TimeResolution", token); break;
							case 4 : map.put("Distribution", token); break;
							case 5 : map.put("Frequency", token); break;
							case 6 : map.put("Target", token); break;
							case 7 : map.put("Priority", token); break;
							case 8 : map.put("NumberOfPackets", token); break;
							case 9 : map.put("PacketSize", token); break;
							case 10: map.put("UniformRate", token); break;
							case 11: map.put("NormalAverageRate", token); break;
							case 12: map.put("NormalMinimalRate", token); break;
							case 13: map.put("NormalMaximalRate", token); break;
							case 14: map.put("NormalStandardDeviation", token); break;
							case 15: map.put("NormalIncrement", token); break;
							case 16: map.put("ParetoRateOnPeriod", token); break;
							case 17: map.put("ParetoBurstSize", token); break;
							case 18: map.put("CTRL_NumberOfPackets", token); break;
							case 19: map.put("GS_NumberOfPackets", token); break;
							case 20: map.put("BE_NumberOfPackets", token); break;
							case 21: map.put("CTRL_PacketSize", token); break;
							case 22: map.put("GS_PacketSize", token); break;
							case 23: map.put("BE_PacketSize", token); break;
							case 24: map.put("CTRL_UniformRate", token); break;
							case 25: map.put("GS_UniformRate", token); break;
							case 26: map.put("BE_UniformRate", token); break;
							default: break;
							}
					    }
					}
	...
	private static void setSceneryParameters(Scenery scenery, Map <String,String> map){
		scenery.setInternalSimulation(Boolean.parseBoolean(map.get("InternalSimulation")));
		scenery.setSimulationTime(Integer.parseInt(map.get("SimulationTime")));
		scenery.setTimeResolution(map.get("TimeResolution"));
