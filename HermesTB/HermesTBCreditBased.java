...
public class HermesTBCreditBased extends NoCGenerationCB {
	...
	private String projectDir, nocDir, scDir, algorithm;
	...
	private void createRouter(){
		...
		try {
			FileInputStream inFile = new FileInputStream(new File(sourceDir + "RouterCC.vhd"));
			BufferedReader buff = new BufferedReader(new InputStreamReader(inFile));

			DataOutputStream data_output = new DataOutputStream(new FileOutputStream(nocDir	+ "RouterCC.vhd"));
			...
			while(line!=null){
				...
				for (int cont = 0; cont < nTokens; cont++) {
					word = st.nextToken();
					if (word.equalsIgnoreCase("algorithm")) {
						word = algorithm;
					} 
					data_output.writeBytes(word);
				}// end for
			...
			JOptionPane.showMessageDialog(null,"Can't write RouterCC.vhd", "Output error",JOptionPane.ERROR_MESSAGE);
			System.exit(0);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null,"Can't write RouterCC.vhd\n"+e.getMessage(), "Error Message",JOptionPane.ERROR_MESSAGE);
			System.exit(0);
		}
	}
