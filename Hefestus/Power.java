...
public class Power extends JFrame implements MouseListener,ActionListener{
    ...
    private JMenuItem mLink,mGlobal,documentation,mOpen;
	...
	public Power(Project project){
		...
		setLocation((resolucao.width-715)/2,(resolucao.height-490)/2);
		setResizable(false);
		setVisible(true);		
	...
	private void calculatePosPowerHandshake(){
		...
		try{
			...
			for(i=0;i<rotx;i++){
				for(j=0;j<roty;j++){
					String address = Convert.formatAddressXY(i, j, 32);
					routers_time = project.getPath() + File.separator + "Power" + File.separator + "Evaluation" + File.separator + "router" + address + ".dat";
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource() == mOpen){
		    ...
		    Help.show("https://corfu.pucrs.br/redmine/projects/atlas/wiki/Power_Evaluation");
