...
/**
 * Generate a Hermes NoC with Credit Based flow control and monitors.
 * @author Aline Vieira de Mello
 * @version
 */
public class CreditBased extends NoCGenerationCB{
	
	private static String sourceDir = Default.atlashome + File.separator + "Hefestus" + File.separator + "Data" + File.separator + "CreditBased" + File.separator;
	 ...
	 */
    public CreditBased(Project project, int simTime, String simResolution){
...
*********************************************************************************/
	...
	private void createNoCMonitor(){
		try{
			...
			//writeMonitorSignals(data_output);
			...
			//writeAllMonitorDefinitions(data_output);
			data_output.writeBytes(Monitor.getInitSignals(Monitor.CB, dimX, dimY, flitSize, 1));
			
			//generate routers port map
			writeAllRoutersPortMap(data_output);
			
			//generate monitors port map
			//writeAllMonitorsPortMap(data_output);
			data_output.writeBytes(Monitor.getPortMap(Monitor.CB, dimX, dimY, flitSize));
	...
	/**
	 * write NoC libraries (add MonitorPackage).
	 * @param data_output 
	 * @throws Exception 
	 */
