...
**    S1     S2      S3             S4 a S7                S8 e S9               S10 a S13             S14 at� size = 0
**
**
**     escrito      => timestamp de sa�da na rede   timestamp de saida do nodo   timestamp de entrada na rede    timestamp de sa�da da rede    lat�ncia    tempo de simula��o
		...
		for(Index = 0; Index<constNumRot;Index++){
			...
			if(inTx(Index)==1){
				...
				if(EstadoAtual[Index] == 1){
					...
					//captura o n�mero de sequencia do pacote
					CurrentFlit[Index] = (unsigned long int)inData(Index);
					fprintf(Output[Index]," %0*X",(int)constFlitSize/4,CurrentFlit[Index]);
					...
					if(Size[Index]==0){
						...
						for(i=0,j=0;i<constFlitSize;i++,j++){
							...
							if(j==constFlitSize/4-1)
							{
								...
								j=-1; //  porque na iteracao seguinte j ser� 0.
							}
						}
						...
						//lat�ncia desde o tempo de cria��o do pacote (em decimal)
